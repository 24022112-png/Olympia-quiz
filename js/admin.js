...
db.ref('answers').orderByChild('timestamp').on('value', (snapshot) => {
  ...
  if (!snapshot.exists()) {
    list.innerHTML = '<p class="text-slate-500 italic text-sm">Chưa nhận được câu trả lời nào...</p>';
    return;
  }
...
// --- CHỨC NĂNG TƯƠNG TÁC GOOGLE SHEET ---
...
function renderGroupInputs(groups = []) {
  ...
  const count = Math.max(groups.length, 5);
